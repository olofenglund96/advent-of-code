...
def get_input_line(line):
    _line = line.split(":")[1]
    return [int(l) for l in _line.strip().split(" ") if l != ""]
...
race_times = get_input_line(lines[0])
race_distance_records = get_input_line(lines[1])


prod = 1
for race_time, record_dist in zip(race_times, race_distance_records):
